...
separated by lightness more reliably than by chroma (see "Red/Orange Separation" below
for the actual L ranges). A different camera might not have this separation and would
need revisiting.

**Factory reference LAB values** are placeholders, not all camera-tuned. Only R and O
were actually measured on this specific unit (centre-square readings, two sessions,
very consistent); W/Y/B/G are borrowed from a different unit entirely and are expected
to be replaced by Save Calibration after a clean scan on your own camera:
...
| `face_scans` | `dict[str, FaceScan]` | dataclass per face (colours + LAB readings) |
...
| `image_size` | `tuple[int, int] \| None` | (width, height) of the last-loaded image, feeds crop slider max |
| `scan_warnings` | `list[str]` | current plain-English warnings |
| `crop_entities` | `dict[str, CropNumberEntity]` | registered crop number entities, keyed by entity key |
| `led_entity_id` | `str \| None` | configured LED light entity ID |
| `led_brightness_entity` | `LedBrightnessEntity \| None` | registered LED Brightness entity |
| `led_stabilise_delay_entity` | `LedStabiliseDelayEntity \| None` | registered LED Stabilise Delay entity |
| `scramble_move_count_entity` | `ScrambleMoveCountEntity \| None` | registered Scramble Move Count entity |
...
The solve result isn't stored in `hass.data` — it only ever exists transiently in the
`rubiks_solved` event payload and in the Solution sensor's own entity state.
...
| 5 | Red | Rotate left | White | Blue | R |
...
| `rubiks.solve` | Run kociemba solver against stored face scans | `{solution, move_count, cube_string}` on success; on failure `{error, solution: "", move_count: 0}` (no `cube_string`) |
...
| `rubiks_scan_rejected` | `{}` | PreviewCropButton |
...
camera: Red L≈14-38, Orange L≈40-63 (`camera_processor.py`'s `_L_WEIGHT` comment — the
single source of truth for these ranges; older revisions of this doc quoted slightly
different numbers, now removed to avoid drifting out of sync again) — only a small gap
between the darkest Orange corner and brightest Red corner, so a/b still matters for
borderline cells even with L weighted higher.
