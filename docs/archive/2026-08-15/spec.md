...
├── config_flow.py        — source selection (camera entity or sample image path), options flow
...
│                           Robot Start Scan, Robot Stop, Robot Abort,
│                           Robot Advance Face; async_handle_solve (shared handler)
├── image.py              — Last Scan, per-face images (W/Y/R/O/B/G), Scan Summary
├── number.py             — Crop Left/Top/Right/Bottom, Grid Rotation, LED Brightness
...
└── tests/
    ├── __init__.py
    ├── conftest.py       — test fixtures, synthetic image generation
    ├── generate_samples.py — generates 13 sample images for CI testing
    ├── samples/          — synthetic test images (3x3 grids, each colour)
    ├── test_camera_processor.py — 16 tests for CIELAB, detection, calibration
    └── test_solver.py    — 16 tests for sticker remapping, kociemba logic
```
...
| W | 66.0 | 16.0 | -15.0 | |
| Y | 74.0 | -1.0 | 26.0 | |
| R | 35.0 | 50.0 | 22.0 | Dark on this camera; centre ~L=33–36 |
| O | 62.0 | 47.0 | 27.0 | Centre ~L=62; corners can fall to L=43 |
| B | 32.0 | 33.0 | -51.0 | |
| G | 45.0 | -15.0 | 14.0 | |

**Why centroid-based, not centre-square-based:** The Red face spans L=16–40 and Orange L=43–66 on this camera. Using centre-only anchors (R at ~L=34, O at ~L=62) places the L decision boundary at (34+62)/2=48 — too high, misclassifying dark Orange corners (L=43) as Red. Centroid-based anchors (R at ~L=35, O at ~L=62) still place the boundary at ~48.5, but the greedy calibration process refines this iteratively, and after several scans the EMA smoothing converges to better boundaries that correctly separate borderline colours.
...
### Phase 3 ESP32 robot component

The robot logic lives in an ESPHome external C++ component (`esphome/components/rubiks_solver/`).
See `docs/robot.md` for full architecture. Key decisions recorded here for completeness:

**Kociemba runs in HA, not on the ESP32.** The two-phase pruning tables need 10–20 MB RAM;
the ESP32-S3 has 8 MB PSRAM. HA already owns the scan pipeline — `kociemba.solve()` is one
extra line in `solver.py`. The ESP32 only executes the solution string it receives.

**Non-blocking step queue.** All servo positions and timings for a scan or solve are pre-computed
into a `std::vector<ServoStep>` at plan time. `loop()` drains the queue one entry per tick using
`App.get_loop_component_start_time()` for timing. Setting `step_start_ms_ = 0` when execution
begins ensures the first step fires on the next `loop()` tick with no artificial delay.

**Duty conversion.** Raw MicroPython PWM duty values (0–1023, from `Cubotino_settings.txt`) are
converted to ESPHome's normalised −1.0..1.0 in `setup()` via `duty_to_esphome()`:
`(duty/1023×100 − 5) / 5 × 2 − 1` for 'small' servos (5%–10% pulse range). The conversion
is C++ only — `__init__.py` handles schema validation and servo-reference codegen, not arithmetic.
...
| `led_entity_id` | `str \| None` | LED entity ID auto-discovered from camera device at config time; override via options flow |
...
|---------|-------------|---------|
| `rubiks.robot_scan_face` | Illuminate → capture → detect → store for one face (`{face: "W"}`) | — |
| `rubiks.solve` | Run kociemba solver against stored face scans | `{solution, move_count, cube_string}` or `{error}` (`SupportsResponse.OPTIONAL`) |

`rubiks.solve` is also called by `SolveButton.async_press()` internally. The response variable pattern lets HA automations read the solution directly without polling the sensor.
...
| `rubiks_solved` | `{solution, move_count, cube_string}` | SolveButton, `rubiks.solve` service |
| `rubiks_calibration_saved` | `{anchors}` | SaveCalibrationButton |
| `rubiks_calibration_reset` | `{}` | ResetCalibrationButton |
| `rubiks_robot_start_requested` | `{}` | RobotStartScanButton |
| `rubiks_robot_stop_requested` | `{}` | RobotStopButton, RobotAbortButton |
| `rubiks_robot_advance_face_requested` | `{}` | RobotAdvanceFaceButton |
